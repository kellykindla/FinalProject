...
After looking at the value counts for the "homeType" column, we decided to focus on single family homes only, as it made up 94% of the housing data. We believed that other home types could skew the model and listing price. 
...
Additional refactoring and cleaning were conducted. All categorical features were encoded and additional extreme outliers were identified and removed from the dataset. Our adjusted r-squared score is 0.728 and we found that for each crime occurrence, the latest listing price decreased by $0.2106. 
